...
enum TypeOfSearch{
  globalSearchWithCity, offersInMainCategory, offersInSubCategory, merchant
}

class SearchScreen extends GetView<SearchOfferController> {
  final TypeOfSearch typeOfSearch;
  final int categoryId;
  SearchScreen({Key? key, required this.typeOfSearch,  required this.categoryId}) : super(key: key);
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: GestureDetector(
              ...
              child: Padding(
                padding:  EdgeInsets.only(top: 10, left: Utils.leftPadding16Left, right: Utils.rightPadding16Right),
                child: Container(
                  width: 37,
                  height: 37,
    ...
    return Obx(() =>  Container(
          ...
          child: TextFormField(
            ...
            decoration: InputDecoration(
                ...
                prefixIcon: Icon(Icons.search_rounded, color: ColorConstants.greyColor, size: 25,),
                suffixIcon: controller.deleteIconShow.value? GestureDetector(
                  onTap: (){
                    _searchController.text ='';
                    controller.searchOffersList.clear();
                    controller.merchantsInSearch.clear();
            ...
            onChanged: (value){
              if(value != '' || value.isNotEmpty ) {
                ...
                if(typeOfSearch == TypeOfSearch.globalSearchWithCity) {
                  ...
                  controller.getOffersInMainCategoryWithCity(mainCatId: categoryId ,searchKeyWord: value);
                }else if(typeOfSearch == TypeOfSearch.offersInSubCategory ) {
                  controller.getOffersInSubCategoryWithCity(subCatId: categoryId,searchKeyWord: value);
  ...
  Widget _buildSearchIcon(){
    return CenterImageForEmptyData(imagePath: 'assets/images/search_empty.png', text: translation.searchIsEmpty.tr);
  }
  ...
  Widget _buildListOfOffers(){
    ...
    (controller.isLoadingSearchOfferList.value && _searchController.text.isNotEmpty)
        ? ListView.builder(
      itemCount: 3,
      ...
      return  _buildOfferSearchedCard( searchModel: controller.searchOffersList[index] , context: context);
    }): Center(child: _buildSearchIcon()) :
    (controller.isLoadingSearchMerchantList.value && _searchController.text.isNotEmpty)?
    ListView.builder(
    itemCount: 3,
    itemBuilder: (context, index) => Padding(
    padding: const EdgeInsets.only(bottom: 16, top: 20),
    ...
    padding: const EdgeInsets.only(bottom: 16, top: 20,left: 16,right: 16),
    itemBuilder: (context, index){
    return  _buildMerchantSearchedCard( searchModel: controller.merchantsInSearch[index] , context: context);
    }): Center(child: _buildSearchIcon()))
    ;
  }
  ...
  Widget _buildOfferSearchedCard(
    ...
    return GestureDetector(
      onTap: (){
          ...
          controller.getOffer(offerKey: searchModel.offerKey.toString()).then((offer) {
            Get.back();
      ...
      child: Container(
        // height: 91,
        padding:  EdgeInsets.only(left: Utils.leftPadding16Left, right: Utils.rightPadding16Right,top: 14, bottom: 14),
        ...
        child: Row(
          ...
          children: [
            Row(
              children: [
                MerchantLogo(
                    merchantLogo: (typeOfSearch == TypeOfSearch.merchant)?  searchModel.companyLogo.toString() : searchModel.offerMainImage.toString().toString(),
                    containerWidth: 60,
                    containerHeight: 55,
                    logoWidth: 33,
                    logoHeight: 33),
                        ...
                        Text(
                            ...
                            translation.offerName.trParams({
                              'offerName': Utils.getTranslatedText(
                                  arText: searchModel.companyArName.toString(),
                                  enText: searchModel.companyEnName.toString())
                            }),
                            //     :
                            //
                            // translation.offerName.trParams({
  ...
  Widget _buildMerchantSearchedCard(
      {
        required MerchantModel searchModel,
        required BuildContext context
      }
      ){
    return GestureDetector(
      onTap: (){
        ...
        controller.getMerchant(merchantKey: searchModel.key.toString()).then((companyModel) {
          Get.back();
      ...
      child: Container(
        // height: 91,
        padding:  EdgeInsets.only(left: Utils.leftPadding16Right, right: Utils.rightPadding16Right,top: 14, bottom: 14),
        margin: const EdgeInsets.only(bottom: 10),
        decoration: const BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.all(Radius.circular(12))),
